 ...
 */
 ...
 .controller('mainController',['$scope', '$location', '$timeout', '$http', 'anchorSmoothScroll', function($scope,$location,$timeout,$http,anchorSmoothScroll) {
     //$scope.layout = window.pageLayout;
     ...
     $scope.form = { resetAcctControl: function() { this.acccd = '' },
     ...
     $scope.policyUrl = $location.absUrl().replace(/[^\/]+$/,"politica-de-privacidade.html");
     $scope.donorPolicyUrl = $location.absUrl().replace(/[^\/]+$/,"politica-de-doacao.html");
     ...
     { 
       ...
       $scope.campaign = $scope.hasValue(urlparams.plia_cpg) ? urlparams.plia_cpg : ( $scope.hasValue(urlparams.utm_campaign) ? urlparams.utm_campaign : ( $scope.hasValue(window.gdCampaign) ? window.gdCampaign : 'Com Os Refugiados' ));
       $scope.pageCode = $scope.hasValue(urlparams.plia_pcode) ? urlparams.plia_pcode : ( $scope.hasValue(window.gdPageCode) ? window.gdPageCode : 'BRPT00GD00 General' );
       var p = urlparams.pid;
       if (p) {
     ...
     $scope.submitForm = function(e,d) {
       ...
       if (d.$valid && contactForm['VALID'].value === 'true' && $scope.payment.getAmount()>=10 && donationForm['TYPE'].value && acceptPolicy && validCaptcha) {
             $scope.posting = true;
         } else {
             var translate = function(s) {
               var translations = {
                 ...
                 'amount' : 'Valor mínimo de doação: R$10',
             ...
             if (!($scope.payment.getAmount()>=10)) msg += '\n - '+translate('amount');
             if (!acceptPolicy) msg += '\n - '+translate('policy');
             if (!validCaptcha) msg += '\n - '+translate('captcha');
             window.alert(msg);
         }
